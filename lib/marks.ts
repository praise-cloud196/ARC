...
export interface RecordMarkInput {
  ...
  note: string;
  artifact?: string;
  /** PRD §9: Mark's optional "source quest" — set when a Mark was generated by completing an Undertaking or achieving an Outcome (milestone-5-spec.md §3, §5). No FK/CHECK enforcement, same trust level as `artifact`. */
...
export async function recordMark(client: Queryable, input: RecordMarkInput): Promise<AppendedEvent> {
  ...
  if (input.artifact !== undefined) payload.artifact = input.artifact;
  if (input.sourceQuestId !== undefined) payload.sourceQuestId = input.sourceQuestId;
