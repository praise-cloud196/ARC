...
- **`dev`** (`br-late-grass-axpmwl76`) — disposable. `.env.local` points here. Reset freely; nothing here is precious. Note: deleting and recreating this branch forks it from `main`'s *current* data rather than giving an empty database — reset the schema (`DROP SCHEMA public CASCADE; CREATE SCHEMA public;` then `npm run migrate`) afterward if a clean slate is needed.

**`dev`'s connection string has died mid-session, unprompted, twice** (both times: `password authentication failed`, fixed by copying a fresh connection string from the Neon console — same role, same password, but a *different* pooler hostname each time). Likely cause, not fully confirmed: Neon does not rotate or expire a role's password on any schedule — rotation is always a manual action (Console, API, or `ALTER ROLE`) ([Neon FAQ](https://neon.com/faqs/rotate-database-connection-string-security)). It does, however, automatically **archive** a branch once it's both more than 14 days old and unaccessed for 24 hours, moving it to cold storage; the next connection auto-triggers an unarchive with "no action required," but Neon's own guide doesn't say what a client sees while that's in flight ([branch archiving](https://neon.com/docs/guides/branch-archiving)). `dev` clears the 14-day threshold easily, and any gap of a day-plus between sessions clears the 24-hour one — a new compute endpoint (hence new pooler hostname) after an idle branch wakes up is consistent with archive/unarchive, though this project's plan tier and whether archiving actually applies to it are unconfirmed from this environment (no Neon console access here). If it happens again: check the branch's state in the [Neon console](https://console.neon.tech/app/projects/gentle-violet-22128593) at the moment of failure, and update this note with what's actually found there.
