...
docs/milestone-4.1-fixes.md §1: the event log is append-only, so development and real use cannot share a database — a mistake made while testing against real data has no undo. Two Neon branches, same project (`gentle-violet-22128593`):

- **`production`** (`br-restless-salad-ax08xdzq`) — real data. Written to only by the deployed app itself (Vercel's own environment variables set `DATABASE_URL`, `ARC_TIMEZONE`, etc. there — never in a repo file) and, deliberately, the real baseline audit, once. Originally created and documented here as `main`; renamed in the Neon console at some point without changing its branch ID, so `ARC_PRODUCTION_BRANCH_ID` below is still correct and nothing downstream broke — confirmed 2026-09-04 by checking the branch's URL in the console directly.
- **`dev`** — disposable. `.env.local` points here. Reset freely; nothing here is precious. Its branch ID isn't pinned in this doc on purpose — unlike `production`, `dev` is expected to be deleted and recreated, so a hardcoded ID here would just go stale again (as the one previously written here did). Note: deleting and recreating this branch forks it from `production`'s *current* data rather than giving an empty database — reset the schema (`DROP SCHEMA public CASCADE; CREATE SCHEMA public;` then `npm run migrate`) afterward if a clean slate is needed. When forking, double-check the database name in the copied connection string is `neondb`: this project also has a second, near-empty database called `arc_restore_scratch_target` (leftover from proving `scripts/restore.ts` against a scratch database, docs/milestone-3-spec.md §0.3) sitting in the same branch, and copying its connection string by mistake looks like a working `dev` right up until the schema turns out to be missing everything past migration `0005`.

**`dev`'s connection string has died mid-session, unprompted, more than once** — at least twice as `password authentication failed` with a different pooler hostname each time (fixed by copying a fresh connection string from the console), and once, on 2026-09-04, as the branch appearing to have vanished from the console entirely. That last one turned out not to be data loss: `production` still had all 52 real events intact, and what actually happened was `dev` no longer existed as a branch at all and had to be recreated from scratch (see above for the scratch-database trap that recreation walked straight into). The earlier password failures remain unexplained by that same incident — the standing hypothesis is still Neon's automatic branch **archiving** (a branch idle 24h+ and older than 14 days moves to cold storage; the next connection auto-unarchives it, which could plausibly present as a fresh pooler endpoint) over scheduled password rotation, since Neon doesn't rotate a role's password on any schedule ([Neon FAQ](https://neon.com/faqs/rotate-database-connection-string-security), [branch archiving](https://neon.com/docs/guides/branch-archiving)) — but this is still unconfirmed, since the one time the console was actually checked, the finding was branch deletion, not archive state. If a plain password failure (branch still visible, just rejecting the password) happens again: check the branch's state in the [Neon console](https://console.neon.tech/app/projects/gentle-violet-22128593) at the moment of failure, and update this note with what's actually found there.

`lib/db-guard.ts` enforces the boundary: `scripts/backup.ts`, `scripts/restore.ts`, `scripts/rebuild.ts`, `scripts/boundary-job.ts`, `scripts/migrate.ts`, and the whole test suite all query `neon.branch_id` from whatever database they're pointed at and refuse to proceed if it matches `ARC_PRODUCTION_BRANCH_ID`, unless `ARC_ALLOW_PRODUCTION_WRITE=1` is set in that specific invocation's environment (same pattern as `ARC_ALLOW_DESTRUCTIVE_MIGRATION` below — deliberately not a casual flag). The one thing that legitimately runs against `production` outside the deployed app is the scheduled production backup (see "Backups"), which sets that flag permanently in its own environment. See `.env.production.example` for what a deliberate one-off run against production (a manual migration, say) needs.
...
The app runs on Vercel, pointed at the Neon `production` branch. Steps:

1. Import this repo into Vercel (dashboard → Add New → Project, or `vercel link` if using the CLI) and select the `main` **git** branch for Production — this is the repo's git branch, unrelated to the Neon database branch of the same-ish name below; don't conflate the two.
2. Set these Production environment variables (Vercel dashboard → Project → Settings → Environment Variables), all real values already in `.env.production` locally:
   - `DATABASE_URL` — the Neon `production` branch's connection string
...
- Project: `gentle-violet-22128593` (`neon.project_id`), branch `br-restless-salad-ax08xdzq` (`neon.branch_id`) — confirmed by querying `pg_settings` directly against the database, and independently by the branch's own URL in the console.
- PITR enabled: **yes** — confirmed 2026-09-04 directly in the console (Branch → Settings → "History retention").
- Retention window: **6 hours**. This is the free-plan default, not something set deliberately — worth knowing that a mistake has to be caught same-day, not "sometime this week."
